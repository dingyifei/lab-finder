...
class TestUniversityDiscoveryAgent:
    ...
    @pytest.mark.skip(
        reason="Removed _fetch_page_with_retry method - now using Claude Agent SDK"
    )
    @pytest.mark.asyncio
    ...
    @pytest.mark.skip(
        reason="WebScraper removed - now using Claude Agent SDK with automatic retry/fallback"
    )
    @pytest.mark.asyncio
