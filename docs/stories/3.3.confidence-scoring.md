...
- [ ] **Task 3: Parse and Validate Confidence Scores** (AC: 1)
  ...
  - [ ] Store validated confidence in Professor.filter_confidence field
...
- [ ] **Task 7: Add Confidence Visualization to Reports** (AC: 4, 6) *[Depends on Task 6]*
...
- Modify: `src/models/professor.py` (add filter_confidence: Optional[float] field)
...
def test_identify_low_confidence_cases():
    # Arrange
    professors = [
        Professor(id="1", name="Dr. Smith", filter_confidence=95, is_relevant=True),
        Professor(id="2", name="Dr. Doe", filter_confidence=65, is_relevant=True),
        Professor(id="3", name="Dr. Lee", filter_confidence=45, is_relevant=False),
    ]
    threshold = 70
    ...
    assert all(p.filter_confidence < threshold for p in borderline)
