...
- [ ] **Task 4: Save Professor List to Checkpoint** (AC: 5, 6)
  ...
  - [ ] **Batch ID Strategy:** Use `batch_id=1` for single consolidated professor list
    ...
    - Recommend single batch for simplicity unless professor count exceeds 500+
...
async def deduplicate_professors(professors: list[Professor]) -> list[Professor]:
    ...
    For large professor lists (100+), consider:
    - Batching fuzzy matches (10-20 at a time)
    - Caching similar name comparisons within same department
    - Fast exact-match pre-filtering reduces LLM calls significantly
    """
