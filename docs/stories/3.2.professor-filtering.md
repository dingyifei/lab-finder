...
- **Epic 1** (Foundation & Configuration Infrastructure)
  - Story 1.4: User Profile Consolidation (provides `output/user-profile.md`)
  - Story 1.7: Shared Utilities Implementation (provides `llm_helpers`, `checkpoint_manager`, `progress_tracker`)
- **Story 3.1**: Multi-Agent Professor Discovery (provides `checkpoints/phase-2-professors-batch-*.jsonl`)
...
- [ ] **Task 2: Update Professor Model with Filter Results** (AC: 1)
  - [ ] Modify existing `src/models/professor.py` to add NEW filter fields:
    ```python
...
- [ ] **Task 3: Implement LLM-Based Professor Filtering** (AC: 1, 2, 3, 4)
  - [ ] Load confidence threshold from `system_params.json` (`confidence_thresholds.professor_filter`) - NOTE: Threshold stored for Story 3.3 use; this story uses LLM decision directly
  ...
  - [ ] Prompt template:
    ...
    ```
  - [ ] Call LLM for each professor with retry logic (3 attempts, exponential backoff)
...
- [ ] **Task 4: Handle Interdisciplinary Researchers** (AC: 3, 5)
...
- [ ] **Task 6: Implement Batch Processing for Filtering** (AC: 1)
  - [ ] Load professors in batches from `checkpoints/phase-2-professors-batch-*.jsonl` (Story 3.1 output)
  - [ ] Process each batch through LLM filtering
  - [ ] Use batch size from `system_params.json` (`batch_sizes.professors`)
...
{
  "batch_sizes": {
    "professors": 20  // Batch size for professor filtering
  },
