...
3. **Test Puppeteer MCP fallback**
   ```python
   @pytest.mark.asyncio
   async def test_puppeteer_mcp_fallback_on_webfetch_failure(mocker):
       # Mock ClaudeSDKClient to raise exception on WebFetch
       mock_client_webfetch = mocker.AsyncMock()
       mock_client_webfetch.__aenter__ = mocker.AsyncMock(side_effect=Exception("WebFetch failed"))

       # Mock ClaudeSDKClient to succeed with Puppeteer MCP
       mock_client_puppeteer = mocker.AsyncMock()
       mock_client_puppeteer.query = mocker.AsyncMock()
       mock_client_puppeteer.receive_response = mocker.AsyncMock(
           return_value=async_iterator_of_mock_messages_with_professor_data()
       )
       mock_client_puppeteer.__aenter__ = mocker.AsyncMock(return_value=mock_client_puppeteer)
       mock_client_puppeteer.__aexit__ = mocker.AsyncMock(return_value=None)

       mocker.patch('claude_agent_sdk.ClaudeSDKClient', side_effect=[mock_client_webfetch, mock_client_puppeteer])
...
|------|---------|-------------|--------|
| 2025-10-09 | 0.4 | **PHASE 3 SPRINT CHANGE PROPOSAL UPDATE** - Updated for Puppeteer MCP fallback: (1) AC#6 updated to reference Puppeteer MCP instead of Playwright; (2) Subtask 3.4 rewritten for Puppeteer MCP fallback with MCP tool examples; (3) Data quality flag changed from `scraped_with_playwright_fallback` to `puppeteer_mcp_used`; (4) Puppeteer MCP Fallback Implementation section rewritten with ClaudeSDKClient + MCP tools pattern; (5) Removed direct Playwright library code (async_playwright, BeautifulSoup selector patterns); (6) Updated test pattern to mock Puppeteer MCP fallback. Story now aligns with Sprint Change Proposal Phase 3 requirements. | Sarah (PO) |
