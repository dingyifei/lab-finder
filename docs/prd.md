...
Develop web scraping capabilities (built-in tools + Playwright fallback) to gather lab websites, publications via paper-search-mcp, member identification, and contact information extraction.

### Epic 5: Publication Research & Member Identification
Integrate paper-search-mcp for PI and member publications (3-year window), abstract and acknowledgment parsing, journal reputation weighting, and lab member inference from co-authorship when websites unavailable.

### Epic 6: LinkedIn Integration & PhD Position Detection
Implement shared Playwright session with subagent queuing, LinkedIn profile access, LLM-based person matching with confidence scores, and graduating/departed PhD detection using configurable graduation duration.
...
#### Story 1.4: User Profile Consolidation
...
**Goal:** Integrate paper-search-mcp to retrieve publications from the last 3 years for PIs and lab members, analyze abstracts and acknowledgments for research alignment and collaboration patterns, and infer lab members from co-authorship when websites are unavailable. This epic delivers comprehensive publication intelligence.
